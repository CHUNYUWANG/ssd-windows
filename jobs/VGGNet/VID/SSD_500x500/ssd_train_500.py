...
train_data = "examples\\VID\\ilsvrc12_train_lmdb"
# The database file for testing data. Created by data/VOC0712/create_data.sh
test_data = "examples\\VID\\ilsvrc12_val_lmdb"
...
model_name = "VGG_VID_{}".format(job_name)

# Directory which stores the model .prototxt file.
save_dir = "models\\VGGNet\\VID\\{}".format(job_name)
# Directory which stores the snapshot of models.
snapshot_dir = "models\\VGGNet\\VID/{}".format(job_name)
# Directory which stores the job script and log file.
job_dir = "jobs\\VGGNet\\VID/{}".format(job_name)
# Directory which stores the detection results.
output_result_dir = "{}\\data_hdd\\results\\VID\\{}\\Main".format('D:\\chnuwa', job_name)
...
name_size_file = "data\\ilsvrc12\\val_name_size.txt"
# The pretrained model. We use the Fully convolutional reduced (atrous) VGGNet.
pretrain_model = "models\\VGGNet\\VGG_ILSVRC_16_layers_fc_reduced.caffemodel"
# Stores LabelMapItem.
label_map_file = "data\\ilsvrc12\\labelmap.txt"
...
neg_pos_ratio = 3.
loc_weight = (neg_pos_ratio + 1. - 2.) / 4.
multibox_loss_param = {
...
with open(job_file, 'w') as f:
  f.write('cd {}\n'.format(caffe_root))
  f.write('caffe.exe train ')
  f.write('--solver="{}" '.format(solver_file))
  f.write(train_src_param)
  if solver_param['solver_mode'] == P.Solver.GPU:
    f.write('--gpu {} 2>&1'.format(gpus))
  else:
    f.write('2>&1')
...
if run_soon:
  ...
  (file_dir, file_name) = os.path.split(job_file)
  subprocess.Popen(file_name, cwd=file_dir, shell=True)
